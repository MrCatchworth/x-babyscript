...
node returns [BabyNode treeNode]
	...
	| leftHand=expr PLUSPLUS
        {
            $treeNode = BabyElement.CreateIncrement($leftHand.fullText);
        } # Element
	| leftHand=expr MINUSMINUS
        {
            $treeNode = BabyElement.CreateDecrement($leftHand.fullText);
        } # Element
	| BLOCK_COMMENT
		{
